...
fn report(source: &str, info: TokenInfo, marker: &str, msg: &str) -> Error {
	miette::miette! {
		...
		"{msg}"
	}.with_source_code(source.to_owned()).into()
}

fn error(parser: &Parser, msg: &str) -> Error {
	report(&parser.source, parser.peek(0).range(), "here", msg)
}
...
fn expected(parser: &Parser, msg: &str) -> Error {
	...
	}.with_source_code(parser.source.to_owned()).into()
}
...
impl Parser<'_,'_> {
	...
	fn expr_call(&mut self, lhs: NodeId, op_token: Token) -> Result<NodeId> {
		...
		if !self.functions.contains(&name) {
			...
			return Err(report(self.source, op_token.range(), "here",
	...
	pub(super) fn stmt_rec(&mut self) -> Result<NodeId> {
		...
		if self.records.contains(&name) {
			return Err(report(self.source, start..end, "here",
	...
	pub(super) fn stmt_var(&mut self) -> Result<NodeId> {
		...
		let name = self.ident()?;
		let vtype = if self.match_token(TokenType::Colon).is_ok() {
			let vtype = self.value_type()?;
			self.match_token(TokenType::Eq1)?;
			vtype
		} else {
			self.match_token(TokenType::Eq1)
				.map_err(|_| expected(self, &format!("type-tag or {}", TokenType::Eq1)))?;
			ValueType::Any
		};
